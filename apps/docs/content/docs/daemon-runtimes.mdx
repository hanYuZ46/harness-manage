...
Create a personal access token in **Settings → Tokens**.
...
| Command | Purpose |
|---|---|
| `harness daemon start` | Start (background by default; add `--foreground` to run in the foreground) |
| `harness daemon stop` | Stop |
| `harness daemon restart` | Restart |
| `harness daemon status` | Show status |
| `harness daemon logs` | Show logs (add `-f` to follow) |

Full CLI reference in [CLI commands](/cli).

**The desktop app ships with a daemon.** It launches the daemon automatically on startup — no need to run `harness daemon start` manually.
...
1. Run `harness daemon status` — confirm the daemon is running and online
2. Run `harness daemon logs -f` — check for errors
