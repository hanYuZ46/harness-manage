...
| 命令 | 作用 |
|---|---|
| `harness daemon start` | 启动（默认后台，加 `--foreground` 前台运行）|
| `harness daemon stop` | 停止 |
| `harness daemon restart` | 重启 |
| `harness daemon status` | 查看状态 |
| `harness daemon logs` | 查看日志（加 `-f` 跟随）|

完整 CLI 参考见 [CLI 命令速查](/cli)。

**桌面应用自带守护进程。**[桌面应用](/desktop-app) 启动时会自动拉起守护进程，无需手动运行 `harness daemon start`。
...
1. 跑 `harness daemon status`，确认守护进程在运行且在线
2. 跑 `harness daemon logs -f`，看是否有错误
