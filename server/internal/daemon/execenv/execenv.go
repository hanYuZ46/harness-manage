...
// checked out on demand by the agent via `harness repo checkout`.
package execenv
...
type PrepareParams struct {
	WorkspacesRoot string            // base path for all envs (e.g., ~/harness_workspaces)
...
// on demand via `harness repo checkout <url>`.
