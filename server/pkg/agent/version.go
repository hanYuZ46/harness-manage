...
// the harness CLI version reported by the daemon at registration time. The
...
// `harness issue create` failure — see PR #1851); older daemons would either
...
var (
	ErrCLIVersionMissing = errors.New("harness CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("harness CLI version is below required minimum")
)
