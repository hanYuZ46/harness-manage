...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Authenticate harness with harness-manager",
}
...
func resolveAppURL(cmd *cobra.Command) string {
	...
	fmt.Fprintln(os.Stderr, "No app URL configured. Run 'harness setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'harness login' to authenticate.")
		return nil
	}
...
<title>harness-manager — Authenticated</title>
<style>
