...
var rootCmd = &cobra.Command{
	Use:   "harness",
	Short: "harness-manager CLI — local agent runtime and management tool",
	Long:  "Work seamlessly with harness-manager from the command line.",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.SetVersionTemplate("harness {{.Version}}\n")
	...
	rootCmd.PersistentFlags().String("server-url", "", "harness-manager server URL (env: HARNESS_SERVER_URL)")
	rootCmd.PersistentFlags().String("workspace-id", "", "Workspace ID (env: HARNESS_WORKSPACE_ID)")
