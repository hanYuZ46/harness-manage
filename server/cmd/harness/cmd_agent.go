...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, HARNESS_SERVER_URL env, or 'harness config set server_url <url>'")
	}
...
func resolveServerURL(cmd *cobra.Command) string {
	...
	fmt.Fprintln(os.Stderr, "No server configured. Run 'harness setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
// ~/.harness/config.json would let the agent act on whatever workspace the
...
func requireWorkspaceID(cmd *cobra.Command) (string, error) {
	...
	if id == "" {
		...
		return "", fmt.Errorf("workspace_id is required: use --workspace-id flag, set HARNESS_WORKSPACE_ID env, or run 'harness config set workspace_id <id>'")
	}
	return id, nil
}
