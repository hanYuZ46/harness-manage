...
[官网](https://harness-manager.ai) · [云服务](https://harness-manager.ai) · [自部署指南](SELF_HOSTING.md) · [参与贡献](CONTRIBUTING.md)

**[English](README.md) | 简体中文**

## harness-manager 是什么？

harness-manager 将编码 Agent 变成真正的队友。像分配给同事一样分配给 Agent——它们会自主接手工作、编写代码、报告阻塞问题、更新状态。
...
这个名字体现了我们的使命：为团队提供工具，将 AI Agent 作为一级团队成员来驾驭。几十年来，软件团队一直处于一种单线程的工作模式，一个工程师处理一个任务，一次只专注于一个上下文。AI agents 改变了这个等式。harness-manager 将"多路复用"带回软件开发，今天系统中进行多路复用的"用户"，既包括人类，也包括自主代理。
...
harness-manager 管理完整的 Agent 生命周期：从任务分配到执行监控再到技能复用。
...
```bash
brew install harness-manager
```

后续可用 `brew upgrade harness-manager` 更新 CLI。
...
如果没有 Homebrew，可以使用安装脚本。脚本会安装 harness-manager CLI：检测到 `brew` 时通过 Homebrew 安装，否则直接下载二进制。
...
```bash
harness setup          # 连接 harness-manager Cloud，登录，启动 daemon
```

> **自部署？** 加上 `--with-server` 在本地部署完整的服务：
>
> ```bash
...
安装好 CLI（或注册 [harness-manager 云服务](https://harness-manager.ai)）后，按以下步骤将第一个任务分配给 Agent：

### 1. 配置并启动 daemon

```bash
harness setup           # 配置、认证、启动 daemon（一条命令搞定）
```

daemon 在后台运行，保持你的机器与 harness-manager 的连接。它会自动检测 PATH 中可用的 Agent CLI（`claude`、`codex`、`copilot`、`openclaw`、`opencode`、`hermes`、`gemini`、`pi`、`cursor-agent`、`kimi`、`kiro-cli`）。

### 2. 确认运行时已连接

在 harness-manager Web 端打开你的工作区，进入 **设置 → 运行时（Runtimes）**，你应该能看到你的机器已作为一个活跃的 **Runtime** 出现在列表中。

> **什么是 Runtime（运行时）？** Runtime 是可以执行 Agent 任务的计算环境。它可以是你的本地机器（通过 daemon 连接），也可以是云端实例。每个 Runtime 会上报可用的 Agent CLI，harness-manager 据此决定将任务路由到哪里执行。
...
在看板上创建一个 Issue（或通过 `harness issue create` 命令创建），然后将其分配给你的新 Agent。Agent 会自动接手任务、在你的 Runtime 上执行、并实时汇报进度——就像一个真正的队友一样。
...
**简单来说：harness-manager 专为团队协作打造，让团队和 AI Agent 一起高效完成项目。**
...
参与 harness-manager 代码贡献，请参阅 [贡献指南](CONTRIBUTING.md)。
